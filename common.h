...
complex dpsi_dt(double x0, double y0, int row, int col, int nrows, int ncols, double mass, double dx, double dy,
	...
	{//RK1
		...
		l = cmul((complex){0, HBAR / (2.0 * mass)}, l);
		complex v_psi = cmul(psi0[current], (complex){0, -v / HBAR});
		r1 = cadd(l, v_psi);
	}
	{//RK2
		double v = V(x, y, t + dt / 2);
		complex l = laplacian(psi0, row, col, nrows, ncols, dx, dy, cmul(r1, (complex){0.5 * dt, 0.0}));
		l = cmul((complex){0, HBAR / (2.0 * mass)}, l);
		complex v_psi = cmul(cadd(psi0[current], cmul(r1, (complex){0.5 * dt, 0})), (complex){0, -v / HBAR});
		r2 = cadd(l, v_psi);		
	}
	{//RK3
		double v = V(x, y, t + dt / 2);
		...
		l = cmul((complex){0, HBAR / (2.0 * mass)}, l);
		complex v_psi = cmul(cadd(psi0[current], cmul(r2, (complex){0.5 * dt, 0})), (complex){0, -v / HBAR});
		r3 = cadd(l, v_psi);	
	}
	{//RK4
		double v = V(x, y, t + dt);
		...
		l = cmul((complex){0, HBAR / (2.0 * mass)}, l);
		complex v_psi = cmul(cadd(psi0[current], cmul(r3, (complex){dt, 0})), (complex){0, -v / HBAR});
		r4 = cadd(l, v_psi);	
	}
